...
namespace chassis
{
     ...
     {    // Sketchy turning with proportional control (that works lol).
          ...
          int distance_to_speed_up = target * 0.2;//make this decrease as the degrees_10 increases
          set_mode(MOTOR_BRAKE_COAST);
          set_voltage(0, 0);
          std::cout << "going into loop 1" << std::endl;
          printf("%d\n", avg_turning_enc_units());
          while (avg_turning_enc_units() < abs(distance_to_speed_up)){
               ratio = (avg_turning_enc_units() / abs(distance_to_speed_up) >= 0.15)
                    ? avg_turning_enc_units() / abs(distance_to_speed_up)
                    : 0.15;
               speed = max_speed * ratio * reverse;
               speed = speed * 1.57 * max_speed / 127;
               set_velocity(speed, -speed);
               pros::delay(20);
               printf("encoderloop1: %f; ratio: %f; speed: %d;\n", avg_turning_enc_units(), ratio,speed);
          }///*
          ...
          while (abs(avg_turning_enc_units() - target) > 25)
               ...
               speed = (proportional * error > max_speed)
                    ? max_speed
                    : proportional * error;
               speed = (speed >= 20)? speed: 20;
               speed = speed * 1.57;
               set_velocity(speed, -speed);
               printf("encoderloop2: %f; target: %d; speed: %d;\n", avg_turning_enc_units(), target,speed);
               pros::delay(20);
          }//*/
          printf("ended loop 2");
